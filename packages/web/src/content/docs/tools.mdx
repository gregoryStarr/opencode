...
Pass `background: true` to launch a long-running command (dev server, watcher, long build) without blocking. It returns immediately with a `shell_id`; the model reads incremental output later with the `bash_output` tool. Background commands have no timeout unless one is set explicitly.

---

### bash_output

Read incremental output from a background command started with `bash` (`background: true`).

Given the `shell_id`, it returns only the output produced since the previous call plus the command's current status (running/completed/error/cancelled). Pass `kill: true` to terminate a running background command.
...
Interact with your configured LSP servers to get code intelligence features like definitions, references, hover info, call hierarchy, and diagnostics.

:::note
This tool is on by default in this build. Set `OPENCODE_DISABLE_LSP_TOOL=true` to turn it off.
:::
...
Supported operations include `goToDefinition`, `findReferences`, `hover`, `documentSymbol`, `workspaceSymbol`, `goToImplementation`, `prepareCallHierarchy`, `incomingCalls`, `outgoingCalls`, and `diagnostics`. The `diagnostics` operation returns the current errors and warnings for a file, so the model can verify an edit compiles cleanly without re-running a full build. `documentSymbol`, `workspaceSymbol`, and `diagnostics` do not require `line`/`character`.
...
:::note
This tool is available for **every provider** in this build (the Exa and Parallel search backends are public). Set `OPENCODE_DISABLE_WEBSEARCH=true` to turn it off. Choose a backend with `OPENCODE_WEBSEARCH_PROVIDER=exa|parallel`; supply `EXA_API_KEY` / `PARALLEL_API_KEY` for higher limits (optional).
:::
...
No API key is required by default — the tool connects directly to the hosted MCP search service without authentication.
